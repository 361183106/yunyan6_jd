/*
年货节红包
cron 0 0,10,20,22 * * *  https://raw.githubusercontent.com/pangbai6/pangbai66/master/jd_nhjRed.js
整点跑 红包几率大点

https://u.jd.com/SthwUCb
跳转到app 可查看助力情况

返利变量：nhjRed_rebateCode，若需要返利给自己，请自己修改环境变量[gua_nhjRed_rebateCode]
SChaCUq换成自己的返利 京粉app转链即可
export jd_nhjRed_rebateCode="SthwUCb"

需要助力[火力值]的账号pin值
如：【京东账号2】pin
pin1换成对应的pin值 用,分开
只助力2个 满了脚本自动从ck1开始替换未满的
export jd_nhjRed_rebatePin="pin1,pin2"
自己改返利码 下面那个是我的
...
let nowTime = new Date().getTime() + new Date().getTimezoneOffset()*1 + 1;
let timeH = $.time('H')
!(async () => {
  if (!cookiesArr[0]) {
        ...
        if(Object.getOwnPropertyNames(shareCodeArr).length > s && timeH != 10 && timeH != 20){
          for(let i in shareCodeArr || {}){
            ...
            if(n == s) {
              ...
              if(res.indexOf('上限') > -1){
                await $.wait(parseInt(Math.random() * 1, 10))
                await getCoupons('',1)
              }
            }
            n++
