/*
cron "1 7-21/2 * * *" jd_Bohnanza.js, tag:种豆得豆日常任务
*/

const $ = new Env('种豆得豆日常任务');
...
async function jdPlantBean() {
  try {
    ...
    if ($.plantBeanIndexResult && $.plantBeanIndexResult.code === '0' && $.plantBeanIndexResult.data) {
      ...
      await doTask();//做日常任务
      // await doEgg();
      await stealFriendWater(); //偷取好友营养液
      await doCultureBean(); //收取营养液
      await doGetReward();//检查是否领取
      await showTaskProcess();//任务进度
      await plantShareSupportList();//助力人数
    } else {
