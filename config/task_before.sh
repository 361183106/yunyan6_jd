...
gen_pt_pin_array() {
    ...
    gen_basic_value(){
        for i in $@; do
            if [[ $i = timestamp ]]; then
                eval export $i="($(def_json_total JD_COOKIE $i | awk '{print $3,$2,$4,$5}' | perl -pe "{s| |-|g}"))"
            else 
                eval export $i='($(def_json_total JD_COOKIE $i | perl -pe "{s| ||g}"))'
            fi
        done
    }
